% 这是一个用于读取PLY格式网格文件的MATLAB函数。PLY是一种常见的三维网格文件格式，包含了网格的拓扑结构和几何信息。
% 该函数可以读取PLY文件中的面片连接信息和顶点位置信息，并可选地读取顶点或面片的颜色信息。
% 
% 函数的输入参数为PLY文件名，输出参数为面片连接信息、顶点位置信息和颜色信息（可选）。函数首先打开文件，
% 读取文件头信息，然后根据文件格式读取文件内容。目前该函数只支持ASCII格式的PLY文件。
% 
% 函数中的read_ascii子函数用于读取ASCII格式的PLY文件。该函数首先读取顶点信息，
% 然后读取面片信息。在读取顶点信息时，函数会跳过注释和空行，然后读取每一行的顶点信息和颜色信息（如果有的话）。
% 在读取面片信息时，函数会跳过注释和空行，然后读取每一行的面片信息和颜色信息（如果有的话）。
% 
